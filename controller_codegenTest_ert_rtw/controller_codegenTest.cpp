...
// C/C++ source code generated on : Mon May 12 17:35:17 2025
//
...
void controller_codegenTest::step()
{
  ...
  //   Inport: '<Root>/state_error'

  yaw_flag = std::abs(rtU.state_error[5]);
  ...
  roll_flag = std::abs(rtU.state_error[4]);
  ...
  roll_flag = std::abs(rtU.state_error[3]);
  if (roll_flag >= 0.087266462599716474) {
    roll_flag = 1.0;
    yaw_flag = 0.0;
  } else if (roll_flag < 0.017453292519943295) {
  ...
  yaw_flag = (2.504 * rtU.state_error[3] - rtDW.Filter_DSTATE_j) * 54.54;
  ...
  pitch_flag = (3.625 * rtU.state_error[4] - rtDW.Filter_DSTATE_k) * 54.54;

  // MATLAB Function: '<S2>/Yaw Cross-Talk Corrector' incorporates:
  //   Inport: '<Root>/state_error'

  if (rtU.state_error[5] > 180.0) {
    roll_flag = rtU.state_error[5] - 360.0;
  } else if (rtU.state_error[5] < -180.0) {
    roll_flag = rtU.state_error[5] + 360.0;
  } else {
    roll_flag = rtU.state_error[5];
  }
  ...
  rtb_yaw_idx_3 = (4.773 * rtU.state_error[3] + rtDW.Integrator_DSTATE_k) +
    yaw_flag;
  ...
  u0 = (6.912 * rtU.state_error[4] + rtDW.Integrator_DSTATE_o) + pitch_flag;
  ...
  rtb_NProdOut_0 = (rtU.state_error[0] * rtb_xyz_idx_2 - rtDW.Filter_DSTATE[0]) *
    rtb_xyz_idx_3;
  ...
  u0_0 = (rtU.state_error[0] * rtb_xyz_idx_0 + rtDW.Integrator_DSTATE[0]) +
    rtb_NProdOut_0;
  if (u0_0 > 80.0) {
  ...
  rtb_NProdOut_0 = (rtU.state_error[1] * rtb_xyz_idx_2 - rtDW.Filter_DSTATE[1]) *
    rtb_xyz_idx_3;
  ...
  u0_0 = (rtU.state_error[1] * rtb_xyz_idx_0 + rtDW.Integrator_DSTATE[1]) +
    rtb_NProdOut_0;
  if (u0_0 > 80.0) {
    // SignalConversion generated from: '<S1>/Maps forces to pwms'
    tmp[1] = 80.0;
  ...
  rtb_NProdOut_0 = (rtU.state_error[2] * rtb_xyz_idx_2 - rtDW.Filter_DSTATE[2]) *
    rtb_xyz_idx_3;
  ...
  u0_0 = (rtU.state_error[2] * rtb_xyz_idx_0 + rtDW.Integrator_DSTATE[2]) +
    rtb_NProdOut_0;
  if (u0_0 > 80.0) {
    // SignalConversion generated from: '<S1>/Maps forces to pwms'
    tmp[2] = 80.0;
  ...
  rtDW.Filter_DSTATE[0] += 0.01 * rtb_NProdOut_idx_0;

  // Update for DiscreteIntegrator: '<S43>/Integrator' incorporates:
  //   Inport: '<Root>/state_error'
  //   Product: '<S40>/IProd Out'

  rtDW.Integrator_DSTATE[0] += rtU.state_error[0] * rtb_xyz_idx_1 * 0.01;

  // Update for DiscreteIntegrator: '<S38>/Filter'
  rtDW.Filter_DSTATE[1] += 0.01 * rtb_NProdOut_idx_1;

  // Update for DiscreteIntegrator: '<S43>/Integrator' incorporates:
  //   Inport: '<Root>/state_error'
  //   Product: '<S40>/IProd Out'

  rtDW.Integrator_DSTATE[1] += rtU.state_error[1] * rtb_xyz_idx_1 * 0.01;

  // Update for DiscreteIntegrator: '<S38>/Filter' incorporates:
  //   Product: '<S46>/NProd Out'

  rtDW.Filter_DSTATE[2] += 0.01 * rtb_NProdOut_0;

  // Update for DiscreteIntegrator: '<S43>/Integrator' incorporates:
  //   Inport: '<Root>/state_error'
  //   Product: '<S40>/IProd Out'

  rtDW.Integrator_DSTATE[2] += rtU.state_error[2] * rtb_xyz_idx_1 * 0.01;

  // Update for DiscreteIntegrator: '<S88>/Filter'
  rtDW.Filter_DSTATE_j += 0.01 * yaw_flag;

  // Update for DiscreteIntegrator: '<S93>/Integrator' incorporates:
  //   Inport: '<Root>/state_error'
  //   Product: '<S90>/IProd Out'

  rtDW.Integrator_DSTATE_k += 2.325 * rtU.state_error[3] * 0.01;

  // Update for DiscreteIntegrator: '<S138>/Filter'
  rtDW.Filter_DSTATE_k += 0.01 * pitch_flag;

  // Update for DiscreteIntegrator: '<S143>/Integrator' incorporates:
  //   Inport: '<Root>/state_error'
  //   Product: '<S140>/IProd Out'

  rtDW.Integrator_DSTATE_o += 3.236 * rtU.state_error[4] * 0.01;

  // Update for DiscreteIntegrator: '<S188>/Filter'
  rtDW.Filter_DSTATE_b += 0.01 * rtb_yaw_idx_2;

  // Update for DiscreteIntegrator: '<S193>/Integrator' incorporates:
  //   Product: '<S190>/IProd Out'

  rtDW.Integrator_DSTATE_a += roll_flag * rtb_yaw_idx_1 * 0.01;
  for (i = 0; i < 8; i++) {
    ...
    if (yaw_flag > 0.0) {
      yaw_flag = rt_roundd_snf(((yaw_flag * yaw_flag * -0.229204832763592 +
        0.00231011300235566 * rt_powd_snf(yaw_flag, 3.0)) + 13.7272237851422 *
        yaw_flag) + 1536.34500151024);
    } else if (yaw_flag < 0.0) {
      yaw_flag = rt_roundd_snf(((yaw_flag * yaw_flag * 0.360062809963915 +
